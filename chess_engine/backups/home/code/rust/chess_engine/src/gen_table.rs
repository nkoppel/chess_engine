...
fn gen_att(sq: usize, deltas: &Vec<(isize, isize)>, board: u64) -> u64 {
    let mut out = 0;
...
fn gen_mask(sq: usize, deltas: &Vec<(isize, isize)>) -> u64 {
    let mut board = 0;
...
fn gen_move_table(deltas: &Vec<(isize, isize)>) -> Vec<u64> {
    let mut out = vec![0; 64];
...
pub struct Tables {
    pub pawn_takes: Vec<u64>,
    pub other_pawn_takes: Vec<u64>,
    pub pawn_moves: Vec<u64>,
...
pub fn new_tables() -> Tables {
    Tables {
        other_pawn_takes: gen_move_table(&vec![(-1, -1), (1, -1)]),
        pawn_takes: gen_move_table(&vec![(-1, 1), (1, 1)]),
        pawn_moves: gen_move_table(&vec![(0, 1)]),
